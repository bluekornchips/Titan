...
const setVendorStatus = async (marketplace_contract: ethers.Contract, token_contract: ethers.Contract, allowed: boolean, marketplace_name: string): Promise<boolean> => {
    Ducky.Debug(__filename, "setVendorStatus", `Setting vendor status of ${await token_contract.name()} ${await token_contract.getAddress()} to ${allowed} on ${marketplace_name}: ${await marketplace_contract.getAddress()}`);
    try {
        ...
        Ducky.Debug(__filename, "setVendorStatus", `Vendor status of ${await token_contract.name()} set to ${allowedStatus} on ${marketplace_name}: ${await marketplace_contract.getAddress()}`);
        return true;
    } catch (error: any) {
