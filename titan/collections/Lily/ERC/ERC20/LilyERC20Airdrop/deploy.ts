...
const deploy = async (name: string, artifact: any): Promise<ethers.Contract> => {
    Ducky.Debug(__filename, "deploy", `Deploying LilyERC20Airdrop: ${name} to ${getActiveEnv().rpc.name}`);
...
const addToPostgres = async (name: string, contract: ethers.Contract, artifact: any) => {
    try {
        const upsertResult = await pgsql.contracts.upsert(name, contract, contract_type_db.Utility, artifact);
        if (!upsertResult) {
