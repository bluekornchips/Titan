...
import {LilyERC721Airdroppable, ILilyERC721Airdroppable} from "Lily/ERC/ERC721/extensions/LilyERC721Airdroppable.sol";
import {LilyERC721TestHelpers} from "test-helpers/LilyERC721TestHelpers.sol";

import {Titan} from "test-helpers/Titan/Titan.sol";

contract LilyERC721Airdroppable_Test is
    Test,
    ILilyERC721Airdroppable,
    LilyERC721TestHelpers,
    Titan
{
    using Strings for uint256;
    using Address for address;
    LilyERC721Airdroppable public lilyERC721Airdroppable;

    function setUp() public {
        lilyERC721Airdroppable = new LilyERC721Airdroppable(
            NAME,
            SYMBOL,
            1_000_000,
            BASE_URI
    ...
    function test_deliverDrop() public {
        ...
        uint16 airDropCounter = lilyERC721Airdroppable._airDropCounter();
        vm.expectEmit(true, false, false, false);
        emit AirdropDelivered(airDropCounter + 1, airdropped);
        lilyERC721Airdroppable.deliverDrop(airdrops);
        ...
        for (uint256 i; i < airdrops.length; i++) {
            ...
            for (uint256 j; j < airdrop.tokenCount; j++) {
                assertEq(
                    lilyERC721Airdroppable.ownerOf(counter++),
                    airdrop.recipient
                );
            }
        }
    ...
    function test_deliverDrop_ManyAddress_ShouldPass() public {
        ...
        lilyERC721Airdroppable.setMaxSupply(totalTokens);
